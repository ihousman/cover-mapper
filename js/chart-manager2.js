...
function makeAreaChart(area,name){
	areaChartingCount++;
	closeChart();
	
	var fColor = randomColor().slice(1,7);
	Map2.addLayer(area,{'palette':fColor},name,true,null,null,name + ' provided Shapefile for summarizing','reference-layer-list');
	updateProgress(50);
	...
	function evalTable(){
		table.evaluate(function(tableT, failure){
			...
			if(failure !== undefined && iteration < maxIterations){evalTable()}
				...
				updateProgress(80);
				 document.getElementById('curve_chart').style.display = 'inline-block';
				  ...
				  uriName = name + ' LCMS_Loss_and_Gain_Summary';
			...
			else{showMessage('Error!',failure)}
			iteration ++;
